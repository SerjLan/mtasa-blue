...
void CPacketHandler::Packet_ServerConnected(NetBitStreamInterface& bitStream)
{
    ...
    {
        g_pCore->ChatEchoColor("* Подключен!", CHATCOLOR_INFO);
    }
            ...
            bitStream.ReadCompressed(usDimension) && bitStream.ReadBit(bIsAttached))
        {
            ...
            switch (ucEntityTypeID)
            {
                ...
                case CClientGame::VEHICLE:
                {
                    ...
                    // √ильдоновские костыли. ѕомен€л char на short, т.к айди неверный приходил!!!
                    ...
                    {
                        // SString strLine2("ќшибка протокола 39 , usModel2: %d ", usModel2;
                        SString strLine("Ошибка протокола 39 , айди тачки %d (до изменений)", usModel);
                        // WriteDebugEvent(strLine2);
                        ...
                        if (usModel + 1788 >= 2000 && usModel + 1788 <= 2200)
                        {
                            ...
                            SString strLine("Ошибка протокола 39 , айди тачки %d (после изменений)", usModel);
                            WriteDebugEvent(strLine);
                        }
                        else
                        {
                            SString strLine("Ошибка протокола 39 , айди тачки %d (я —ƒќ’Ћј » ¬џЋ≈“≈Ћј)", usModel);
