...
#define PORT_TO_AUTOCONNECTZBT 22022
#define PORT_TO_AUTOCONNECT 22023
...
CMainMenu::CMainMenu(CGUI* pManager)
{
   ...
   // m_pVersion->SetProperty("InheritsAlpha", "False");
    ...
    m_pDisconnect = CreateItem(MENU_ITEM_DISCONNECT, "menu_disconnect.png", CVector2D(fButt, fBase + fGap * 3));
    m_pDisconnect->image->SetVisible(false);

    // Create the menu items
    // Filepath, Relative position, absolute native size
    // And the font for the graphics is ?
    m_menuItems.push_back(CreateItem(MENU_ITEM_QUICK_CONNECT, "menu_quick_connect.png", CVector2D(fButt, fBase + fGap * 3)));
    m_menuItems.push_back(CreateItem(MENU_ITEM_QUICK_CONNECTZBT, "menu_map_editor.png", CVector2D(fButt, fBase + fGap * 4)));
 ...
 //   m_menuItems.push_back(CreateItem(MENU_ITEM_HOST_GAME, "menu_host_game.png", CVector2D(0.168f, fBase + fGap * 2)));
   // m_menuItems.push_back(CreateItem(MENU_ITEM_MAP_EDITOR, "menu_map_editor.png", CVector2D(0.168f, fBase + fGap * 3)));
    m_menuItems.push_back(CreateItem(MENU_ITEM_SETTINGS, "menu_settings.png", CVector2D(fButt, fBase + fGap * 5)));
   // m_menuItems.push_back(CreateItem(MENU_ITEM_ABOUT, "menu_about.png", CVector2D(0.168f, fBase + fGap * 5)));
    m_menuItems.push_back(CreateItem(MENU_ITEM_QUIT, "menu_quit.png", CVector2D(fButt, fBase + fGap * 6)));
...
bool CMainMenu::OnMenuClick(CGUIMouseEventArgs Args)
{
	...
	if (Args.button != LeftButton && m_pHoveredItem->menuType != MENU_ITEM_QUICK_CONNECTZBT)
        return true;
    ...
    switch (m_pHoveredItem->menuType)
    {
        ...
        case MENU_ITEM_QUICK_CONNECT:
            ...
            break;        
		case MENU_ITEM_QUICK_CONNECTZBT:
            OnQuickConnectButtonClickZbt(pElement, Args.button == LeftButton);
            break;
...
bool CMainMenu::OnQuickConnectButtonClickZbt(CGUIElement* pElement, bool left)
{
    // Return if we haven't faded in yet
    if (m_ucFade != FADE_VISIBLE)
        return false;
    ...
    std::string nicknameply;
    CVARS_GET("nick", nicknameply);
    CCore::GetSingleton().GetConnectManager()->Connect(IP_TO_AUTOCONNECT, PORT_TO_AUTOCONNECTZBT, nicknameply.c_str(), PASSWORD_TO_AUTOCONNECT);
    
    m_ServerBrowser.SetVisible(false);
    m_ServerBrowser.OnQuickConnectButtonClickZbt();
    return true;
}
